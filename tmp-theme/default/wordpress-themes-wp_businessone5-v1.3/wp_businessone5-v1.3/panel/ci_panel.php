...
function ci_register_admin_scripts() 
{
	...
	//
	wp_register_script('ci-colorpicker', get_template_directory_uri().'/panel/scripts/colorpicker/js/colorpicker.js', array('jquery'));
	wp_register_style('ci-colorpicker', get_template_directory_uri().'/panel/scripts/colorpicker/css/colorpicker.css');

	wp_register_script('ci-panel', get_template_directory_uri().'/panel/scripts/panelscripts.js', array('jquery'));
	wp_register_style('ci-panel-css', get_template_directory_uri().'/panel/panel.css');

	wp_register_script('ci-post-formats', get_template_directory_uri() .'/panel/scripts/ci-post-formats.js', array('jquery'));
	wp_register_style('ci-post-formats', get_template_directory_uri() .'/panel/styles/ci-post-formats.css');

	wp_register_script('ci-media-manager-3-3', get_template_directory_uri().'/panel/scripts/media-manager-3.3.js', array('thickbox'));
	wp_register_script('ci-media-manager-3-5', get_template_directory_uri().'/panel/scripts/media-manager-3.5.js', array('media-editor'));
...
function ci_create_menu() {
	...
	if(!CI_WHITELABEL)
		add_theme_page(__('CSSIgniter Settings', 'ci_theme'), __('CSSIgniter Settings', 'ci_theme'), 'edit_theme_options', basename(__FILE__), 'ci_settings_page');
	else
		add_theme_page(__('Theme Settings', 'ci_theme'), __('Theme Settings', 'ci_theme'), 'edit_theme_options', basename(__FILE__), 'ci_settings_page');
}
...
{ ?>
	<div class="wrap">
		...
		<?php if(($latest_version !== false) and ($latest_version > CI_THEME_VERSION)): ?>
			<div id="theme-update">
			...
			<form method="post" action="options.php" id="theform" enctype="multipart/form-data">
				...
				<div id="ci_header">
					<?php if(!CI_WHITELABEL): ?>
						<img src="<?php echo get_child_or_parent_file_uri('/panel/img/logo.png'); ?>" />
					<?php endif; ?>
				</div>
...
function ci_panel_input($fieldname, $label, $params=array())
{
	...
	<input id="<?php echo $fieldname; ?>" type="text" size="60" name="<?php echo THEME_OPTIONS.'['.$fieldname.']'; ?>" value="<?php echo esc_attr($ci[$fieldname]); ?>" class="<?php echo $params['input_class']; ?>" />
	<?php
}
