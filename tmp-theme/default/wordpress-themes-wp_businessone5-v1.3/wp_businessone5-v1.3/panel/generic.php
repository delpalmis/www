 ...
 * Uses get_posts() so parameters and return values are the same.
 * Depends on sort_query_by_post_in() hooked to 'posts_orderby' in order to preserve the order of the IDs.
 *
 * @param array $arr_1 A valid get_posts() parameters' array.
 * @param array $arr_2 A valid get_posts() parameters' array.
 * @param array $arr_n A valid get_posts() parameters' array.
...
function merge_wp_queries()
{
	...
	foreach($args as $arg)
	{
		...
		$this_posts = get_posts($arg);

		foreach($this_posts as $p)
			$merged[] = $p->ID;
	}
...
function ci_theme_update_check()
{
	...
	$update_period = apply_filters('ci_theme_update_period', 60*60*24);
